...
govet, staticcheck and `unused` all run from inside golangci-lint --
one binary, configured in `.golangci.yml`. Its govet default set is
generated from `cmd/vet`'s own list, so there is no separate `go vet`
step; its staticcheck is configured to honnef's check set rather than
the redistributed default.
